...
router.post('/', [ tokenAuth,
    ...
    if (skills) profileFields.skills = skills.split(',').map(
        skill => skill.trim()
    );
...
// @access Private
router.delete('/experience/:exp_id', tokenAuth, async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.user.id });
...
// @access Private
router.delete('/education/:edu_id', tokenAuth, async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.user.id });
